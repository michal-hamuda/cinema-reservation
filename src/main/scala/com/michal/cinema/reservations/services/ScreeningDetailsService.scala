...
import com.michal.cinema.util.CinemaError
...
class ScreeningDetailsService(
  ...
  def get(screeningId: ScreeningId): Future[Either[CinemaError, ScreeningDetails]] = {
    db.run(getDbio(screeningId).map(_.toRight(CinemaError.NotFound)))
  }
